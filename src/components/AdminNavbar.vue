<template>
  <div class="admin-nav">
    <nav class="narrow-screen-nav">
      <div class="narrow-screen-nav__nav-toggle">
        <input type="button" name="nav" id="navbar" @click="toggleNav" />
        <label for="navbar" :class="{ 'label-active': isNavVisible }"
          >導覽列</label
        >
      </div>
    </nav>
    <div
      class="nav-container"
      :style="{ display: isNavVisible ? 'block' : '' }"
    >
      <div class="icon">
        <svg
          width="50"
          height="50"
          viewBox="0 0 50 50"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
            d="M30.6996 23.4709C30.6996 23.4709 23.2328 35.5781 17.1109 35.5781C6.40314 35.5781 16.3821 12.5132 24.1916 12.5132C29.641 12.5132 30.6996 23.4709 30.6996 23.4709Z"
            fill="#FF6600"
          />
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
            d="M39.5723 38.6972C37.4451 40.9607 34.8198 42.6976 31.9044 43.7701C28.989 44.8426 25.8639 45.2213 22.7767 44.8761C19.6895 44.5309 16.7252 43.4713 14.1188 41.7813C11.5125 40.0914 9.33573 37.8175 7.76123 35.1401C9.5314 37.5477 12.5338 39.1527 16.803 39.1527C27.3199 39.1527 33.2899 30.98 33.2899 30.98C33.2899 30.98 33.8062 37.4392 39.5593 38.6972H39.5723Z"
            fill="#FF6600"
          />
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
            d="M42.7223 15.7451C44.2963 18.7527 45.0781 22.1116 44.9939 25.505C44.9096 28.8984 43.9621 32.2144 42.2407 35.1401C40.8792 34.3764 39.8725 33.1082 39.4379 31.609L37.1558 24.7724L42.7223 15.7451Z"
            fill="#FF6600"
          />
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
            d="M25.0031 5.01737C30.0185 5.01403 34.8513 6.89881 38.5398 10.2967L34.7521 16.4088C33.3811 12.843 30.5957 9.2729 25.1767 9.2729C14.0784 9.2729 5.85231 21.3888 5.85231 28.9846C5.84986 29.9669 5.97528 30.9454 6.22543 31.8953C5.11558 28.8725 4.75205 25.6265 5.16572 22.4331C5.57938 19.2398 6.75802 16.1934 8.60147 13.5531C10.4449 10.9127 12.8987 8.75634 15.7542 7.26726C18.6097 5.77818 21.7826 5.00039 25.0031 5V5.01737Z"
            fill="#FF6600"
          />
        </svg>
      </div>
      <div class="nav-group">
        <div class="tweets-list" @click="adminList">
          <div class="tweets-icon">
            <svg
              width="24"
              height="22"
              viewBox="0 0 24 22"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                v-if="$route.name === 'tweets-list'"
                d="M22.58 6.34986L12.475 0.896855C12.178 0.736855 11.821 0.736855 11.525 0.896855L1.42496 6.34986C0.938957 6.61386 0.757957 7.21986 1.01996 7.70586C1.19996 8.04086 1.54496 8.23086 1.89996 8.23086C2.05996 8.23086 2.22396 8.19286 2.37496 8.11086L3.10896 7.71486L4.69896 18.9649C4.91496 20.1789 6.00896 21.0269 7.35896 21.0269H16.641C17.991 21.0269 19.085 20.1789 19.303 18.9389L20.891 7.71386L21.628 8.11186C22.113 8.37486 22.72 8.19386 22.982 7.70786C23.245 7.22186 23.062 6.61486 22.578 6.35286L22.58 6.34986ZM12 14.4349C10.205 14.4349 8.74996 12.9799 8.74996 11.1849C8.74996 9.38986 10.205 7.93486 12 7.93486C13.795 7.93486 15.25 9.38986 15.25 11.1849C15.25 12.9799 13.795 14.4349 12 14.4349Z"
                fill="#FF6600"
              />
              <path
                v-else
                d="M22.58 6.34986L12.475 0.896855C12.178 0.736855 11.821 0.736855 11.525 0.896855L1.42496 6.34986C0.938957 6.61386 0.757957 7.21986 1.01996 7.70586C1.19996 8.04086 1.54496 8.23086 1.89996 8.23086C2.05996 8.23086 2.22396 8.19286 2.37496 8.11086L3.10896 7.71486L4.69896 18.9649C4.91496 20.1789 6.00896 21.0269 7.35896 21.0269H16.641C17.991 21.0269 19.085 20.1789 19.303 18.9389L20.891 7.71386L21.628 8.11186C22.113 8.37486 22.72 8.19386 22.982 7.70786C23.245 7.22186 23.062 6.61486 22.578 6.35286L22.58 6.34986ZM12 14.4349C10.205 14.4349 8.74996 12.9799 8.74996 11.1849C8.74996 9.38986 10.205 7.93486 12 7.93486C13.795 7.93486 15.25 9.38986 15.25 11.1849C15.25 12.9799 13.795 14.4349 12 14.4349Z"
                fill="#000000"
              />
            </svg>
          </div>
          <span
            class="words"
            :style="{
              color: $route.name === 'tweets-list' ? '#FF6600' : '#000000',
            }"
          >
            推文清單
          </span>
        </div>
        <div class="users-list" @click="adminUsers">
          <div class="users-icon">
            <svg
              width="18"
              height="21"
              viewBox="0 0 18 21"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                v-if="$route.name == 'users-list'"
                d="M8.99991 10.8158C10.3549 10.8158 11.8719 10.6658 12.8399 9.55976C13.6539 8.62976 13.9179 7.19176 13.6459 5.16776C13.2659 2.34276 11.5289 0.655762 8.99991 0.655762C6.47091 0.655762 4.73391 2.34276 4.35391 5.16976C4.08191 7.19176 4.3459 8.62976 5.1599 9.55976C6.1279 10.6668 7.64491 10.8158 8.99991 10.8158ZM5.83991 5.36776C6.00191 4.16776 6.62691 2.15576 8.99991 2.15576C11.3729 2.15576 11.9979 4.16876 12.1599 5.36776C12.3669 6.91776 12.2169 7.99476 11.7099 8.57276C11.2549 9.09276 10.4439 9.31576 8.99991 9.31576C7.55591 9.31576 6.7449 9.09276 6.28991 8.57276C5.78291 7.99476 5.63291 6.91676 5.83991 5.36776ZM17.2799 18.2358C16.4029 14.7098 12.9979 12.2458 8.99991 12.2458C5.00191 12.2458 1.59691 14.7098 0.719905 18.2358C0.547905 18.9278 0.691905 19.6358 1.1149 20.1758C1.5229 20.6958 2.15491 20.9958 2.84791 20.9958H15.1519C15.8449 20.9958 16.4769 20.6958 16.8849 20.1758C17.3089 19.6358 17.4519 18.9288 17.2789 18.2358H17.2799ZM15.7039 19.2518C15.5779 19.4118 15.3879 19.4978 15.1519 19.4978H2.84791C2.61291 19.4978 2.42191 19.4128 2.29591 19.2518C2.15891 19.0778 2.11591 18.8398 2.17591 18.5978C2.88591 15.7428 5.69291 13.7478 8.99991 13.7478C12.3069 13.7478 15.1139 15.7418 15.8239 18.5978C15.8839 18.8398 15.8409 19.0778 15.7039 19.2518Z"
                fill="#FF6600"
              />
              <path
                v-else
                d="M8.99991 10.8158C10.3549 10.8158 11.8719 10.6658 12.8399 9.55976C13.6539 8.62976 13.9179 7.19176 13.6459 5.16776C13.2659 2.34276 11.5289 0.655762 8.99991 0.655762C6.47091 0.655762 4.73391 2.34276 4.35391 5.16976C4.08191 7.19176 4.3459 8.62976 5.1599 9.55976C6.1279 10.6668 7.64491 10.8158 8.99991 10.8158ZM5.83991 5.36776C6.00191 4.16776 6.62691 2.15576 8.99991 2.15576C11.3729 2.15576 11.9979 4.16876 12.1599 5.36776C12.3669 6.91776 12.2169 7.99476 11.7099 8.57276C11.2549 9.09276 10.4439 9.31576 8.99991 9.31576C7.55591 9.31576 6.7449 9.09276 6.28991 8.57276C5.78291 7.99476 5.63291 6.91676 5.83991 5.36776ZM17.2799 18.2358C16.4029 14.7098 12.9979 12.2458 8.99991 12.2458C5.00191 12.2458 1.59691 14.7098 0.719905 18.2358C0.547905 18.9278 0.691905 19.6358 1.1149 20.1758C1.5229 20.6958 2.15491 20.9958 2.84791 20.9958H15.1519C15.8449 20.9958 16.4769 20.6958 16.8849 20.1758C17.3089 19.6358 17.4519 18.9288 17.2789 18.2358H17.2799ZM15.7039 19.2518C15.5779 19.4118 15.3879 19.4978 15.1519 19.4978H2.84791C2.61291 19.4978 2.42191 19.4128 2.29591 19.2518C2.15891 19.0778 2.11591 18.8398 2.17591 18.5978C2.88591 15.7428 5.69291 13.7478 8.99991 13.7478C12.3069 13.7478 15.1139 15.7418 15.8239 18.5978C15.8839 18.8398 15.8409 19.0778 15.7039 19.2518Z"
                fill="black"
              />
            </svg>
          </div>
          <span
            class="words"
            :style="{
              color: $route.name === 'users-list' ? '#FF6600' : '#000000',
            }"
            >使用者列表</span
          >
        </div>
        <div class="log-out" @click="logOut">
          <svg
            width="20"
            height="18"
            viewBox="0 0 20 18"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M9 13L5 9M5 9L9 5M5 9H19M14 13V14C14 14.7956 13.6839 15.5587 13.1213 16.1213C12.5587 16.6839 11.7956 17 11 17H4C3.20435 17 2.44129 16.6839 1.87868 16.1213C1.31607 15.5587 1 14.7956 1 14V4C1 3.20435 1.31607 2.44129 1.87868 1.87868C2.44129 1.31607 3.20435 1 4 1H11C11.7956 1 12.5587 1.31607 13.1213 1.87868C13.6839 2.44129 14 3.20435 14 4V5"
              stroke="black"
              stroke-width="2"
              stroke-linecap="round"
              stroke-linejoin="round"
            />
          </svg>
          <span>登出</span>
        </div>
      </div>
    </div>
  </div>
</template>

<style lang="scss" scoped>
img,
svg {
  cursor: initial;
}

.admin-nav {
  flex-basis: 378px;
  position: relative;
  font-family: "Noto Sans TC", sans-serif;
  font-size: 18px;
  font-weight: 700;
  color: #1c1c1c;

  * {
    box-sizing: border-box;
  }

  /* narrow screen nav only shows up when device width is less than 769px */
  .narrow-screen-nav {
    display: none;
  }

  a {
    text-decoration: none;
  }

  span {
    margin-left: 20px;
  }

  .nav-container {
    padding: 4px min(30px, calc(30 / 1440 * 100vw)) 17px
      min(113px, calc(113 / 1440 * 100vw));

    .icon {
      width: 50px;
      height: 50px;
      margin-top: 4px;
    }

    .nav-group {
      display: flex;
      flex-direction: column;
      margin-top: 20px;

      .tweets-list,
      .users-list,
      .log-out {
        display: flex;
        align-items: center;
        margin: 10px;
        cursor: pointer;
      }

      svg {
        cursor: pointer;
      }

      .log-out {
        position: absolute;
        bottom: 17px;
      }
    }
  }
}

@media all and (max-width: 768px) {
  .admin-nav {
    .narrow-screen-nav {
      display: flex;
      justify-content: space-around;

      > div {
        padding-left: 1.2em;
        text-align: center;
        position: relative;

        > input {
          // cancel default button style to hide it
          // cuz display: none causes :focus selector cannot be triggered
          all: unset;
        }

        // normal label UI
        label::before {
          content: "";
          border-top: 0.4em solid transparent;
          border-right: 0.4em solid transparent;
          border-bottom: 0.4em solid transparent;
          border-left: 0.4em solid #0f0f0f;
          position: absolute;
          top: calc(50% - 0.4em);
          left: 0.5em;
        }

        // change label style when narrow screen nav is clicked
        .label-active {
          color: #ffa061;
          text-decoration: underline 0.1em #ffa061;
          text-underline-offset: 0.1em;

          // change the label::before style when input element is clicked
          &::before {
            transform-origin: 20% 50%;
            transform: rotate(90deg);
            border-left-color: #ffa061;
          }
        }
      }
    }

    .nav-container {
      display: none; // only shows up when user clicks narrow screen nav
      position: absolute;
      z-index: 1;
      height: calc(100vh - 31px);
      flex: initial;
      margin: 0;
      padding: 0;
      background-color: white;
      width: 100%;
      text-align: center;

      .icon {
        margin-inline: auto;
      }

      .nav-group {
        width: initial;
        height: initial;
        align-items: center;
      }
    }
  }
}
</style>

<script>
export default {
  data() {
    return {
      // control the visibility of navbar on narrow screen device
      isNavVisible: false,
    };
  },
  methods: {
    adminList() {
      this.$router.push({ name: "tweets-list" });
    },
    adminUsers() {
      this.$router.push({ name: "users-list" });
    },
    logOut() {
      localStorage.removeItem("token");
      // 成功登出後到 SignIn
      this.$router.push("/");
    },
    toggleNav() {
      if (document.body.clientWidth <= 768) {
        if (!this.isNavVisible) {
          this.isNavVisible = true;
        } else {
          this.isNavVisible = false;
        }
      }
    },
  },
};
</script>
